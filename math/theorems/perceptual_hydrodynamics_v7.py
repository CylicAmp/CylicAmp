...
Sovereign links:
  R=0.3252 ≈ 12/37 → 37R ≈ 12 (sovereign target); DR(12)=3
  ...
  53200 mod 37=31; DR(31)=4 (sovereign anchor); DR(53200)=1 (identity seed)
"""
...
# R ≈ 12/37: sovereign target 12 over sovereign prime 37
R_SOVEREIGN = 12 / 37
assert abs(R_COMPRESS - R_SOVEREIGN) < 0.001    # within 0.1%
assert dr(12) == 3                               # 12 has DR=3 (sovereign target)
...
# 11 twin prime: (11, 13); DR(11)=2, DR(13)=4 (sovereign anchor)
assert is_prime(OBS)
assert is_prime(OBS + 2)        # 13 is prime: twin pair
assert dr(OBS + 2) == 4         # DR(13) = 4 (sovereign anchor)
...
assert dr(31) == 4                       # DR(31) = 4 (sovereign anchor)
...
# DR(4836) = 4+8+3+6 = 21 → DR=3 (sovereign target!)
assert dr(4836) == 3
...
# Madelung–Sovereign link:
# The phase S corresponds to 37-field angle: S → 2π × (n/37) for n in F₃₇
# At the sovereign fixed point n=30: S = 2π × 30/37
S_sovereign = 2 * math.pi * 30 / 37
psi_sovereign = math.sqrt(rho_val) * cmath.exp(1j * S_sovereign)
assert abs(abs(psi_sovereign)**2 - rho_val) < 1e-12   # density preserved at fixed point
...
# R=0.3252 ↔ 9×9 Sovereign Matrix: compression = 1 − 12/37 = 25/37
assert abs(1 - R_SOVEREIGN - 25/37) < 1e-12
...
if __name__ == "__main__":
    ...
    print(f"     R ≈ 12/37 = {R_SOVEREIGN:.4f}  (sovereign target 12, DR={dr(12)}) ✓")
    ...
    print(f"     Twin prime: (11, 13); DR(13) = {dr(13)} (sovereign anchor) ✓")
    print()
    print(f"  V. 53,200-Year Cycle (gap = {GAP}):")
    print(f"     53200 = 2⁴×5²×7×19 ✓")
    print(f"     53200 mod 37 = {CYCLE_YEARS%37}, DR({CYCLE_YEARS%37}) = {dr(CYCLE_YEARS%37)} (sovereign anchor) ✓")
    print(f"     DR(53200) = {dr(CYCLE_YEARS)} (identity seed) ✓")
    print(f"     53200/11 = {RATIO:.1f} → DR(4836) = {dr(4836)} (sovereign target 3) ✓")
    ...
    print(f"     Sovereign phase S=2π×30/37 preserves ρ ✓")
    print()
