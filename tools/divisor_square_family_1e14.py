...
STATE=f"chunk2_{sys.argv[1]}.json"
...
def big_ext(c,s2,comps,hits):
    pool=Counter()
    for (p,e) in comps: pool.update(fac(s2pe(p,e)))
    seen=set()
    def ext(depth,m,s2m,pool,chosen):
        if depth==DEPTH: return
        for q in list(pool):
            if q<T or any(q==a for a,_ in chosen): continue
            pe=1; e=0
            while True:
                pe*=q; e+=1
                if m*pe>M: break
...
def node(j,c,s2,comps,hits):
    check(c,s2,hits)
    if s2/(c*c)*tail[-1]>=1.5: big_ext(c,s2,comps,hits)
...
def subtree(item):
    ...
    def dfs(j,c,s2,comps):
        n[0]+=1; node(j,c,s2,comps,hits)
        for ch in children(j,c,s2,comps): dfs(*ch)
    dfs(*item)
...
if __name__=="__main__":
    ...
    if i>=len(items):
        ...
        for m,q in hits:
            ...
            c[tau]+=1
        print("COMPLETE. tau:",dict(sorted(c.items())))
